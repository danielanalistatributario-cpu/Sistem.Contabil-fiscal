// Monta o .xlsx do relatório "NF-e de Entrada e Saída" (layout do modelo
// original do cliente), a partir das linhas já extraídas por
// buildRelatorioNFeRows (sped-nfe-report.ts). Extraído de
// api/sped/relatorio-nfe/route.ts pra ser chamado tanto no navegador
// (Conversor SPED processa o arquivo local, sem upload — ver
// [[analise-apuracao-fiscal-modulo]]) quanto, se um dia precisar, do
// servidor.
...
// Uint8Array (não Buffer, API só do Node) — esta função roda no navegador
// (Conversor SPED processa o arquivo localmente, sem upload). Confirmado
// que workbook.xlsx.writeBuffer() funciona e devolve dado compatível nesse
// ambiente. NextResponse e Blob aceitam Uint8Array direto.
export async function gerarRelatorioNFeExcel(rows: RelatorioNFeRow[]): Promise<Uint8Array> {
