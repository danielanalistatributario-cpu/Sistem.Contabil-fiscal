...
export async function POST(req: NextRequest) {
  ...
  const itensPersistidos = await prisma.analiseFiscalItem.count({ where: { apuracaoId } });
  ...
  await prisma.analiseFiscalApuracao.update({ where: { id: apuracaoId }, data: { status: 'CONCLUIDA' } });
