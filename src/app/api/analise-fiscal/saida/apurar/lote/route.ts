...
import type { ItemApuradoSaida } from '@/lib/analise-fiscal-saida-compute';
...
export async function POST(req: NextRequest) {
  ...
  const itens: ItemApuradoSaida[] = Array.isArray(body?.itens) ? body.itens : [];
  ...
  await prisma.analiseFiscalSaidaItem.createMany({ data: itensData });
  ...
  if (divergenciasData.length > 0) {
    await prisma.analiseFiscalSaidaDivergencia.createMany({ data: divergenciasData });
  }
